...
--INSERTING LOAN
DECLARE
    v_resource_id NUMBER := X; -- Replace X with the resourceID
    v_copy_id NUMBER := Y; -- Replace Y with the copyID
    v_user_id NUMBER := Z; -- Replace Z with the userID
BEGIN
...
--RETURNING LOAN
DECLARE
    v_loan_id NUMBER := X; -- Replace X with the loanID
    v_user_id NUMBER := Y; -- Replace Y with the userID
BEGIN
    -- Update Loan
    UPDATE Loan
    ...
    UPDATE ResourceCopy rc
    SET loanable = 0
    WHERE copyID = (SELECT copyID FROM Loan WHERE loanID = v_loan_id);
 
    -- Update Student
    UPDATE Student
    SET resourcesBorrowed = resourcesBorrowed - 1
...
--MAKING RESERVATION
DECLARE
    v_resource_id NUMBER := X; -- Replace X with the actual reservationID
    v_user_id NUMBER := Y; -- Replace Y with the actual userID
BEGIN
INSERT INTO RESERVATIONDETAILS (reservationID, resourceID, userID, queue, pickupDeadline, missedPickups)
VALUES (
    reservation_id_sequence.NEXTVAL,
          	v_resource_id, -- Replace X with the resourceID
          	v_user_id, -- Replace Y with the userID
          	(SELECT COALESCE(MAX(queue), 0) + 1 FROM RESERVATIONDETAILS WHERE resourceID = v_resource_id),
          	CASE WHEN (SELECT COALESCE(MAX(queue), 0) FROM RESERVATIONDETAILS WHERE resourceID = v_resource_id) = 0 THEN SYSDATE + 2 ELSE NULL END, -- Set pickupDeadline if queue is 0
          	0
);
END;
/

--MISSING PICKUP FOR RESERVATION
DECLARE
    v_reservation_id_1 NUMBER := X; -- Replace X with Missed reservation
    v_reservation_id_2 NUMBER := Y; -- Replace Y next reservationID
 
    v_queue_position_1 NUMBER;
    v_queue_position_2 NUMBER;
BEGIN
    -- Retrieve current queue positions
    SELECT queue INTO v_queue_position_1 FROM ReservationDetails WHERE reservationID = v_reservation_id_1;
    SELECT queue INTO v_queue_position_2 FROM ReservationDetails WHERE reservationID = v_reservation_id_2;
 
    -- Update queue positions
    UPDATE ReservationDetails
    SET queue = v_queue_position_2
    WHERE reservationID = v_reservation_id_1;
 
    UPDATE ReservationDetails
    SET queue = v_queue_position_1
    WHERE reservationID = v_reservation_id_2;
 
    -- Example: Manually increment missedPickups for a specific reservation
    UPDATE ReservationDetails
    SET missedPickups = missedPickups + 1
    WHERE reservationID = v_reservation_id_1;
END;
/
...
DECLARE
    v_loan_id NUMBER := X; -- Replace X with the loanID
    v_fine NUMBER := Y; -- Replace Y with the amount of fine
BEGIN
...
FROM
    Student
JOIN
    Loan ON Student.userID = Loan.userID
LEFT JOIN
    FineDetails ON Loan.loanID = FineDetails.loanID;

--UPDATE FINE
UPDATE FineDetails
SET fine = fine + X -- or - if paying off loan
WHERE loanID = Y;

--LOANS WITH ADD DETAILS
SELECT
    u.userID,
    u.firstName,
    u.lastName,
...
JOIN
    ResourceCopy rc ON l.copyID = rc.copyID AND l.resourceID = rc.resourceID
JOIN
    Resources r ON l.resourceID = r.resourceID
WHERE
    rc.loanable = 1;

--RESERVATIONS WITH ADD DETAILS
SELECT
    r.resourceID,
    r.name AS resourceName,
    rd.reservationID,
...
JOIN
    Resources r ON rd.resourceID = r.resourceID
JOIN
    Student s ON rd.userID = s.userID;
...
SELECT
    ...
    u.firstName,
    u.lastName,
    COUNT(rd.reservationID) AS pendingReservations
FROM (
    SELECT
        s.userID,
        s.firstName,
...
LEFT JOIN
    ReservationDetails rd ON u.userID = rd.userID
WHERE
    rd.queue > 0
GROUP BY
    u.userID, u.firstName, u.lastName
ORDER BY
    pendingReservations DESC;
...
SELECT
    ...
    u.firstName,
    u.lastName,
    COALESCE(SUM(fd.fine), 0) AS totalFines
FROM
    (
        SELECT userID, firstName, lastName FROM Student
        UNION
        SELECT userID, firstName, lastName FROM Staff
    ) u
LEFT JOIN Loan l ON u.userID = l.userID
LEFT JOIN FineDetails fd ON l.loanID = fd.loanID
GROUP BY
    u.userID,
    u.firstName,
    u.lastName
HAVING
    COALESCE(SUM(fd.fine), 0) > 0;
